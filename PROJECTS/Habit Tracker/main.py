...
root.iconphoto(True, PhotoImage(file="snorlax.png"))
root.resizable(width=False, height=False)
root.config(pady=20, padx=20,bg="sky blue")
URL = "https://pixe.la/v1/users/hexon337/graphs/graph1.html"
TODAY = dt.now()
...
def add_pixel():
    endpoint = "https://pixe.la/v1/users/hexon337/graphs/graph1/"
    pixel_add = {
        "date": format_date(),
        "quantity": user_in.get(),
    }
    r=requests.post(url=endpoint, json=pixel_add, headers=headers)
    print(r.text)
...
def del_pixel():
    endpoint = f"https://pixe.la/v1/users/hexon337/graphs/graph1/{format_date()}"
